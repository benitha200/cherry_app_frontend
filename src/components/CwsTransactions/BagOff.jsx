...
const BagOff = ({ token, cwsname, cwscode, cws, profile }) => {
    ...
    if ((profile.jobTitle.includes('CWS') && profile.jobTitle.includes('Manager'))) {
      ...
      if (startdate && enddate) {
        ...
        try {
          ...
          const response = await fetch("http://192.168.81.68:8000/api/retrieveprocessingallstations/", requestOptions)
          const data = await response.json();
